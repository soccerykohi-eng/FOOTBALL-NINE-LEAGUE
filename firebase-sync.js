...
import { initializeApp, getApps, getApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
import { getAuth, GoogleAuthProvider, signInAnonymously, signInWithRedirect, signOut, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";
import { getFirestore, doc, setDoc, onSnapshot, serverTimestamp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
const auth = getAuth(app);
const db = getFirestore(app);
const ADMIN_EMAIL = "soccerykohi@gmail.com";
const googleProvider = new GoogleAuthProvider();
const leagueRef = doc(db, "shared", window.FNL_FIREBASE_APP_ID || "fnl-season1");
let applyingRemote = false;
let unsubscribe;

window.FNL_IS_ADMIN = false;
const setAdminUi = (isAdmin, email = "") => {
  window.FNL_IS_ADMIN = isAdmin;
  document.getElementById("admin-login-button")?.toggleAttribute("hidden", isAdmin);
  document.getElementById("admin-logout-button")?.toggleAttribute("hidden", !isAdmin);
  document.getElementById("admin-logout-button")?.setAttribute("title", email || "");
  window.dispatchEvent(new Event("fnl-admin-state"));
};

window.fnlAdminSignIn = async () => {
  try {
    await signInWithRedirect(auth, googleProvider);
  } catch (error) {
    console.error("Admin sign-in failed", error);
    alert("ログインを開始できませんでした。Firebase Authentication の Google ログイン設定を確認してください。");
  }
};

window.fnlAdminSignOut = async () => {
  await signOut(auth);
  await signInAnonymously(auth);
};
...
state.dbSaveFn = async () => {
  if (applyingRemote) return;
  if (!window.FNL_IS_ADMIN) throw new Error("Administrator sign-in required");
  const updatedAt = new Date().toISOString();
  state.lastSavedAt = updatedAt;
  saveStateToStorage();
  if (!auth.currentUser) await signInAnonymously(auth);
...
onAuthStateChanged(auth, (user) => {
  const isAdmin = Boolean(user && !user.isAnonymous && user.email?.toLowerCase() === ADMIN_EMAIL);
  if (user && !user.isAnonymous && !isAdmin) {
    alert("このGoogleアカウントは運営アカウントとして登録されていません。");
    signOut(auth).then(() => signInAnonymously(auth));
    return;
  }
  setAdminUi(isAdmin, user?.email || "");
  setStatus(isAdmin ? "運営ログイン中" : "閲覧モード", true);
  startListening();
});
